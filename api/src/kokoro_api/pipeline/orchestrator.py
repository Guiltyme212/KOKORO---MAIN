...
from kokoro_api.providers.audio.base import AudioResult, MeditationAudioProvider, StreamReady
...
PipelineStreamEvent = StreamScriptEvent | StreamStreamingEvent | StreamReadyEvent
...
async def run_pipeline_streaming(
    ...
    """Same pipeline as `run_pipeline`, but yields three events as work
    progresses so the client can advance UI as soon as the streaming audio
    URL is ready (~20-40s) instead of waiting for the final mastered audio
    (~90-180s)."""
    t0 = time.monotonic()
    meditation_id = str(uuid.uuid4())
    ...
    bound.info("pipeline.start", streaming=True)
    ...
    async for event_type, payload in deps.audio.synthesize_streaming(
        script=written.lyrics,
        voice_persona_id="",
        music_style_prompt=written.style,
        reference_track_urls=[str(u) for u in template.reference_track_urls],
        target_duration_sec=template.target_duration_sec,
        locale=input.locale,
        candidates=2,
        ...
        if event_type == "stream":
            assert isinstance(payload, StreamReady)
            bound.info(
                "pipeline.audio_streaming",
                stream_audio_url=payload.stream_audio_url,
                duration_sec=payload.duration_sec,
                candidate_id=payload.candidate_id,
                job_id=payload.job_id,
            )
            stream_emitted = True
            yield StreamStreamingEvent(
                meditation_id=meditation_id,
                stream_audio_url=payload.stream_audio_url,
                duration_sec=payload.duration_sec,
            )
            ...
            bound.info(
                ...
                duration_sec=payload.duration_sec,
                suno_latency_ms=payload.latency_ms,
                suno_jobs=payload.job_ids,
                chosen_candidate=payload.chosen_candidate,
            )

    if final_audio is None:
        raise RuntimeError("audio provider produced no 'final' event")
    if not stream_emitted:
        # Provider doesn't have a streaming stage (or fell through to default);
        # emit a streaming event from the final audio so the client always
        # gets one before 'ready'. The blob URL isn't ready yet, so reuse
        # nothing — this branch shouldn't fire for sunoapi.org in practice.
        yield StreamStreamingEvent(
            meditation_id=meditation_id,
            stream_audio_url="",
            duration_sec=final_audio.duration_sec,
        )
    ...
    meta: dict[str, object] = {
        ...
        "lyrics": written.lyrics,
        "estimatedDurationSec": written.estimated_duration_sec,
        "audioDurationSec": final_audio.duration_sec,
        "validationWarnings": written.validation_warnings,
    }
    ...
    persisted = await persist(
        PersistInput(
            meditation_id=meditation_id,
            audio_bytes=final_audio.audio_bytes,
            mime_type=final_audio.mime_type,
            meta=meta,
        ),
        deps.blob,
    )
    ...
    provider_meta = ProviderMeta(
        ...
        audio=AudioMeta(
            provider=deps.audio.name,
            job_id=",".join(final_audio.job_ids),
            latency_ms=final_audio.latency_ms,
            candidates=final_audio.candidate_count,
            chosen_candidate=final_audio.chosen_candidate,
        ),
        persistence=PersistenceMeta(provider=deps.blob.name, latency_ms=persisted.latency_ms),
        total_latency_ms=int((time.monotonic() - t0) * 1000),
    )
