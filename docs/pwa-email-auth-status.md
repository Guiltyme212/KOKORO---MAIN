...
- Удаление аккаунта не отзывает уже выданные токены: другие устройства того
  ...
  устройствах.
- ~~PwaUpdatePrompt посреди входа~~ — сделано: на gate-экранах (login/
  checkingAccess/accessRequired/installApp) обновление SW применяется
  молча; verifyEmail исключён (перезагрузка стёрла бы вводимый код);
  внутри приложения остаётся баннер.
